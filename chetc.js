function Epigraph2Markup(replacements) {
	return {
        id: new String("00"),
        lineNum: new String("01"),
        replacementStr: replacements,
        ignoreLB: false,
        lineBreaks: function (ignore) {
            if (ignore == 'true') {
                this.ignoreLB = true;
            } else {
                this.ignoreLB = false;
        ...
        convert: function (text) {
            ...
            for (i = 0; i < this.replacements.length; i++) {
                ...
                while (ids = result.match(/id="%mkID(\d)"/)) {
                    if (parseFloat(this.id) < 9) {
                        this.id = '0' + (parseFloat(this.id) + 1)
                    }
                    else {
                        this.id = '' + (parseFloat(this.id) + 1)
                    }
                    this.id = 'gap' + this.id;
                    var idPattern = new RegExp('(id=")%mkID' + ids[1] + '(")');
                    result = result.replace(idPattern, "$1" + this.id + "$2");
                    if (replace.match(/target="%mkID\d"/)) {
                        var targPattern = new RegExp('(target=")%mkID' + ids[1] + '(")');
                        result = result.replace(targPattern, "$1" + this.id + "$2");
                    }
                    this.id = this.id.substr(3);
                }
                while (lines = result.match(/lb n="%mkID(\d)"/)) {
                    this.lineNum = '' + (parseFloat(this.lineNum) + 1)
                    var idPattern = new RegExp('(lb n=")%mkID' + lines[1] + '(")');
                    result = result.replace(idPattern, "$1" + this.lineNum + "$2");
                    if (replace.match(/target="%mkID\d"/)) {
                        var targPattern = new RegExp('(target=")%mkID' + lines[1] + '(")');
                        result = result.replace(targPattern, "$1" + this.lineNum + "$2");
                    }
                }
            }
            return result;
